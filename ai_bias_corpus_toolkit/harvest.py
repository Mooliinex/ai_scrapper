...
def main():
    ...
    ensure_dir(args.out)
    since = dtp.parse(args.since)
    until = dtp.parse(args.until)
    rate = conf.get("rate_limit",{}).get("sleep_seconds", 1.0)
    # RSS (generic + NGO)
    total = 0
    total += harvest_rss(conf["sources"].get("rss",[]), since, until, rate, args.out, label="news")
    total += harvest_rss(conf["sources"].get("ngo_rss",[]), since, until, rate, args.out, label="ngo")
    # OpenAlex
    total += harvest_openalex(conf["sources"].get("openalex",{}), since, until, rate, args.out)
    # GDELT
    total += harvest_gdelt(conf["sources"].get("gdelt",{}), since, until, rate, args.out)
